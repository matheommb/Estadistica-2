...
def myAllRegTable(df, target_col):
    ...
    all_combinations = [c for i in range(len(cols)+1) for c in combinations(cols, i)]
    ...
    for comb in all_combinations:
        ...
        cp = (sse_p / mse) - (n - 2*(len(comb) + 1))
        cp_p = cp - (len(comb) + 1)
        reg_results.append([len(comb), r_sq, adj_r_sq, mse, cp, cp_p, comb])
    
    col_names = ['k', 'R_sq', 'adj_R_sq', 'MSE', 'Cp', 'Cp_p', 'Variables_in_model']
    result = pd.DataFrame(reg_results, columns=col_names)
    result = result.drop_duplicates(subset=['Variables_in_model']).reset_index(drop=True)
    return result
