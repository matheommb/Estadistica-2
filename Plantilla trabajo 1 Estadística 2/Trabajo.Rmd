...
$$Y_i = \beta_o + \beta_1 X_{1i} + \beta_2 X_{2i} + \beta_3 X_{3i} + \beta_4 X_{4i} + \beta_5X_{5i}+ \varepsilon_i, \ \varepsilon_i \stackrel{\text{iid}}{\sim} N(0, \sigma^2); \ 1 \leqslant i \leqslant 55$$
...
$$\hat{Y}_i = `r betas[1]` + `r betas[2]` X_{1i} `r betas[3]` X_{2i} + `r betas[4]` X_{3i} +`r betas[5]` X_{4i} + `r betas[6]`X_{5i} +\varepsilon_i, \ \varepsilon_i \stackrel{\text{iid}}{\sim} N(0, \sigma^2); \ 1 \leqslant i \leqslant 50$$
...
tabla.anova %>% 
  ...
  kable_styling(latex_options = c("HOLD_position"))
```
De la tabla Anova, se observa un valor P aproximadamente igual a 0, por lo  que se rechaza la hipótesis nula en la que $\beta_j = 0$ con $1 \leqslant j \leqslant 5$, aceptando la hipótesis alternativa en la que algún $\beta_j \neq 0$, por lo tanto la regresión es significativa.
...
tabla.coeficientes %>% 
  ...
  kable_styling(latex_options = c("HOLD_position"))
```
Los P-valores presentes en la tabla permiten concluir que con un nivel de significancia $\alpha = 0.05$, los parámetros $\beta_3$ y $\beta_5$ son significativos, pues sus P-valores son menores a $\alpha$.
...
El  modelo tiene un coeficiente de determinación múltiple $R^2 = 0.4475$, lo que significa que aproximadamente el $44.75\%$ de la variabilidad total  observada en la respuesta es explicada por el modelo de regresión propuesto en el presente informe.
...
$$Y_i = \beta_0 + \beta_3 X_{3i} + \beta_5 X_{5i} + \varepsilon; \ \varepsilon_i \stackrel{\text{iid}}{\sim} N(0, \sigma^2); \ 1 \leqslant i \leqslant 55$$
...
Ahora, comparando el $F_0$ con $f_{0.95, 3, `r nrow(datos)-6`} = `r round(qf(0.95, 3, nrow(datos)-6), 4)`$, se puede ver que $F_0 ? f_{0.95, 1, 45}$
...
\begin{cases}
\begin{aligned}
\text{H}_0&: \beta_1 = 3\beta_3; \ \beta_2  = \beta_4 \\
...
$$Y_i = \beta_o  + \beta_2 X^*_{2i}+ \beta_3 X^*_{3i} + \beta_5 X_{5i}+\varepsilon_i, \ \varepsilon_i \stackrel{\text{iid}}{\sim} N(0, \sigma^2); \ 1 \leqslant i \leqslant 50$$
...
\begin{equation}
F_0 = \frac{(SSE(MR) - SSE(MF))/2}{MSE(MF)} \stackrel{H_0}{\sim} f_{2, `r nrow(datos)-6`}\\
\end{equation}
